...
from calc_app.models import (FlatHouseWithWaterAnalysis, 
...
class NumberFlatForm(forms.ModelForm):

    action = forms.CharField(max_length=20, widget=forms.HiddenInput(), initial='flat_form')
    water_hardness = forms.ChoiceField(choices=[(0, 'до 3'), (3, 'до 7'), (7, 'от 7')], label='жесткость')
    water_ferum = forms.ChoiceField(choices=[(0, 'до 0,6'),(0.6, 'до 0,9'), (0.9, 'от 0,9')], label='железо')
...
class NumberHouseForm(forms.ModelForm):
    ...
    water_v_used_per_hour = forms.ChoiceField(choices=[
        ...
        label='людей в доме')
    water_hardness = forms.ChoiceField(choices=[
            (0, 'до 3'),
            (3, 'до 8'),
            (8, 'до 15'),
            (20, 'от 15')], 
        label='жесткость')
    water_ferum = forms.ChoiceField(choices=[
            (0, 'до 0,3'),
            (0.3, 'до 0,9'),
            (0.9, 'до 8'),
            (8, 'от 8')],
...
class BoolFlatForm(forms.ModelForm):
...
class BoolCountryHouseForm(forms.ModelForm):
...
class BoolBaseHouseForm(forms.ModelForm):
